...
def generate_second_layer_weight(graph: MultiGraph, graph_order: int, graph_size: int, graph_c_size: int) -> nn.Parameter:
    second_layer_weight = torch.zeros(graph_order, graph_order + graph_size + graph_c_size)
...
def generate_second_layer_biases(graph_order: int, graph_size: int, graph_c_size: int) -> nn.Parameter:
...
def generate_third_layer_weight(graph_order: int, graph_size: int, graph_c_size: int) -> nn.Parameter:
...
class DatalessNet(nn.Module):
    def __init__(
        ...
        self.graph_c_size = (self.graph_order * (self.graph_order - 1)) // 2 - self.graph_order

        self.temperature = 0.5

        self.gamma = self.graph_order

        self.theta_layer = ConstrainedElemMultiply(in_features=self.graph_order, out_features=self.graph_order)
        with torch.no_grad():
        ...
        self.layer2.weight = generate_second_layer_weight(G, self.graph_order, self.graph_size, self.graph_c_size)
        self.layer2.bias = generate_second_layer_biases(self.graph_order, self.graph_size, self.graph_c_size)
        ...
        self.layer3.weight = generate_third_layer_weight(self.graph_order, self.graph_size, self.graph_c_size)
    ...
    def forward(self, x: Tensor) -> Tensor:
        with torch.no_grad():
            self.layer3.weight.data[self.graph_order + self.graph_size : ] = -1.0 * self.temperature
    ...
    def update_gamma(self) -> None:
        with torch.no_grad():
            self.layer3.weight.data[self.graph_order : self.graph_order + self.graph_size] = self.gamma
